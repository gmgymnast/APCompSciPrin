...
class Manager:
     def __init__(self): 
        ...
        self.player_turn = 0 if self.turn == 0 else 1  # 0 = player, 1 = computer
        self.player_score = 0
        self.computer_score = 0
        self.mode = random.randint(0, 1)
        self.computer_mode = "dumb" # "dumb" if self.mode == 0 else "smart"
        self.lose = "None"
...
class Marble:
    ...
    def draw_marble(self, numMar):
        ...
        title_surface = play_font.render(title_text, True, (0, 0, 0))
        title_rect = title_surface.get_rect(center = ((1280 / 2, 125)))
        screen.blit(title_surface, title_rect)

        for i in range((numMar // self.num_rows) + 1): 
            if i != (numMar // self.num_rows):
                for j in range(self.num_rows): screen.blit(marble, (((width / 2) + (j * self.mmx) - ((self.mmx * self.num_rows) / 2)), ((height / 1.8) + (i * self.mmy) - ((self.mmy * self.num_columns) / 2)), self.marble_width, self.marble_height))
            else:
                for j in range(numMar % self.num_rows): screen.blit(marble, (((width / 2) + (j * self.mmx) - ((self.mmx * self.num_rows) / 2)), ((height / 1.8) + (i * self.mmy) - ((self.mmy * self.num_columns) / 2)), self.marble_width, self.marble_height))
...
class Main:
    ...
    def draw_computer(self):
        if self.manager.computer_mode == "dumb": computer_mode_text = "The Computer is in Dumb Mode"
        if self.manager.computer_mode == "smart": computer_mode_text = "The Computer is in Smart Mode"
        ...
        computer_mode_surface = play_font.render(computer_mode_text, True, (0, 0, 0))
        title_surface = play_font.render(title_text, True, (0, 0, 0))
        number_marbles_surface = play_font.render(number_marbles_text, True, (0, 0, 0))

        computer_mode_rect = computer_mode_surface.get_rect(center = ((1280 / 2, 20)))
        ...
        screen.blit(computer_mode_surface, computer_mode_rect)
        screen.blit(title_surface, title_rect)
    ...
    def draw_player(self):
        ...
        title_surface = play_font.render(title_text, True, (0, 0, 0))
        number_marbles_surface = play_font.render(number_marbles_text, True, (0, 0, 0))
        number_marbles_taken_by_computer_surface = play_font.render(number_marbles_taken_by_computer_text, True, (0, 0, 0))
    ...
    def computer_move(self):
        if self.manager.player_turn == 1:
            if self.manager.computer_mode == "dumb": self.marbles_taken = random.randint(1, int(self.manager.numMar / 2)) if self.manager.numMar > 1 else 1
            elif self.manager.computer_mode == "smart":
                if self.manager.numMar < 2: 
                    print("You Win!")
                    sys.exit()
                    exit()
                else: self.marbles_taken = self.manager.numMar - (2 ** int(math.log(self.manager.numMar, 2)) - 1)
            self.manager.numMar -= self.marbles_taken
            self.manager.computer_score += self.marbles_taken
            self.marbles_taken_by_computer = self.marbles_taken
            self.manager.number_marbles_text = 0
            self.manager.player_turn = 0
                
    def player_move(self):
        if self.manager.player_turn == 0:
            title_surface = play_font.render(self.manager.title_text, True, (0, 0, 0))
            number_marbles_surface = play_font.render(str(self.manager.number_marbles_text), True, (0, 0, 0))
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN: 
                    if pygame.key.name(event.key) in "123456789": self.manager.number_marbles_text = (self.manager.number_marbles_text  * 10) + int(pygame.key.name(event.key))
                if event.type == pygame.KEYDOWN and pygame.key.name(event.key) == "return":
                    if self.manager.number_marbles_text != 0:
                        if self.manager.numMar != 1 and self.manager.number_marbles_text > self.manager.numMar / 2: self.manager.number_marbles_text = 0
                        else:
                            self.manager.numMar -= self.manager.number_marbles_text
                            self.manager.player_score += self.manager.number_marbles_text
                            if self.manager.numMar == 0: 
                                print("You Lose!")
                                sys.exit()
                                exit()
...
class Menu:
    ...
    def draw_menu(self):
        ...
        title_surface = menu_main_font.render(title_text, True, (0, 0, 0))
        regular_mode_surface = play_font.render(regular_mode_text, True, (0, 0, 0))  
...
class StateChange():
    ...
    def change_state(self):
        if self.manager.game_state == "Menu": self.menu.draw_elements()
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if self.manager.game_state == "Menu":
                    if event.key == pygame.K_1: self.manager.game_state = "Game"           
